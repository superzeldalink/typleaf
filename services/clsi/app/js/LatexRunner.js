...
function runLatex(projectId, options, callback) {
  ...
  const imageToUse =
    compiler === 'typst' && !image ? Settings.clsi?.typstImage : image
...
function _buildLatexCommand(mainFile, opts = {}) {
  if (opts.compiler === 'typst') {
    return _buildTypstCommand(mainFile, opts)
  }
...
function _buildTypstCommand(mainFile, opts = {}) {
  const command = ['typst', 'compile', '--root', '$COMPILE_DIR']
